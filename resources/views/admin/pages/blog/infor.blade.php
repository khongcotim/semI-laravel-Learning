                    ...
                    <div class="card-body px-0 pb-0">
                        ...
                        <div class="post-navigation d-flex justify-content-between py-4">
                            <a title="See Previous Blog"
                                href="{{ $prev_blog != null ? route('blog.show', $prev_blog->id) : 'javascript:void(0)' }}"
                            ...
                            <a title="See Next Blog"
                                href="{{ $next_blog != null ? route('blog.show', $next_blog->id) : 'javascript:void(0)' }}"
                                    ...
                                    <div class="card-img">
                                        ...
                                        <div class="card-body">
                                            <h3 class="card-title line-height-26"><a
                                                    href="{{ route('blog.show', $related->id) }}">{{ $related->title }}</a>
                                            </h3>
                                            <p class="card-meta">
                                                <span class="post__date">
                            ...
                            @foreach ($reply_cmt[$cmt->id] as $reply)
                                @if ($reply->admin_name != '')
                                    <div class="comment comment-reply-item">
                                        ...
                                        <div class="comment-body">
                                            <div class="meta-data">
                                                <h3 class="comment__author">{{ $reply->admin_name }} <small style="color: grey">Admin</small></h3>
                                                <div class="meta-data-inner">
                                                    <p class="comment__date">
                                        ...
                                        @if (Auth::check())
                                            @if ($reply->admin_reply == Auth::user()->id)
                                            <form action="{{ route('reply.destroy', $reply->id) }}"
                                                ...
                                                <div class="comment-footer" style="right: 0;position: absolute">
                                                    <a class="btn btn-primary" style="text-align: right;color: #fff"
                                                            onclick="edit_reply(this.id)" id="{{ $reply->id }}">Edit</a>
                                                        @csrf
                                                        @method('DELETE')
                                                        <button id="delete_btn{{ $reply->id }}"
                            ...
                            <div class="tab-pane show active" id="new" role="tabpanel" aria-labelledby="new-tab">
                                @foreach ($new_post as $new)
                                    <div class="card-item card-item-list recent-post-card mb-0">
                                        <div class="card-img">
                                            <a href="{{ route('blog.show', $new->id) }}" class="d-block">
                                        ...
                                        <div class="card-body">
                                            <h3 class="card-title"><a
                                                    href="{{ route('blog.show', $new->id) }}">{{ $new->title }}</a>
                                            </h3>
                                            <p class="card-meta">
                                                <span class="post__date">
...
<script>
    function checkLength(id_reply) {
        var messLength = ($('#message_edit' + id_reply).val()).length;
    ...
    function edit_reply(id_reply) {
        ...
        $('#message_edit' + id_reply).removeClass('d-none');
        $('#' + id_reply).hide(200);
    ...
    function cancer_reply(id_reply) {
        ...
        $('#message_edit' + id_reply).addClass('d-none');
        $('#' + id_reply).show(200);
