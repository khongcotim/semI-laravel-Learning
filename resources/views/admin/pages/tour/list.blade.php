                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="form-box">
                            ...
                            <div class="form-content pb-2">
                                <div class="card-item card-item-list ">
                                    <div class="card-item justify-content-between d-flex align-items-center p-2">
                                        <input class="title checkbox" type="checkbox" name="tourChecked[]" id=""  value="{{$value->id}}">
                                    </div>
                                    <div class="card-img">
                            ...
                            @endforeach
                            <div class="form-content pb-2">
                                <button id="tour-check" onclick="checked()" class="theme-btn theme-btn-small theme-btn-transparent" type="submit">Delete all tour checked</button>
                            </div>
                        </div><!-- end form-box -->
                    </div><!-- end col-lg-12 -->
                </div><!-- end row -->
...
<script>
    function checked(){
        ...
        if (isChecked == true) {
            $('#tour-check').attr('type','submit')
            $('#formBlade').attr('action', "{{route('tour.destroyByCheck')}}");
        }else{
            $('#tour-check').attr('type','button')
            Swal.fire(
                'Error',
                'Please choose least one tour to delete',
                'warning'
            )
        }
    }
...
@section('searchForm')
<form action="{{route('tour.index')}}" method="GET" id="formBlade">
@csrf
@endsection
